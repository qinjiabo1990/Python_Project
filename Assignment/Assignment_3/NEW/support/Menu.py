"""Solutions to the week 10 tutorial questions
about simple graphics programming and event handling.
...
class SettingsFrame(tk.Frame) :
    ...
    def set_position(self, x, y) :
        ...
        self._position_label.config(text="Current Position: ({}, {})"
                                         .format(x, y))
...
class DrawingApp(object) :
    ...
    def evt_motion(self, event) :
        ...
        if self._start is not None :
            ...
            if self._settings.is_preview_on() :
                self._canvas.create_line(self._start, (event.x, event.y))
    ...
    def evt_click(self, event) :
        ...
        if self._start is None :
            ...
            self._lines.append((self._start, (event.x, event.y)))
            self._start = None
            self.redraw()
