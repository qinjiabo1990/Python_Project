...
#Ask user physical characteristics question and judging weather the answer is valid
...
#Ask user personality question and judging weather the answer is valid
...
#Matching gender and sexual preference at first, if there are two or more results, matching the personality score and height and height preference
def match(gender, sexual_pref, height, height_pref, personality_score):
    ...
    closest_score = float("inf")    #infinity
    while potential_partners.available() :
        if  (result[1] == potential_partners.get_gender()
            ...
            if abs(result[4] - potential_partners.get_personality_score()) < closest_score: 
                closest_score = abs(result[4] - potential_partners.get_personality_score()) #absolute value
